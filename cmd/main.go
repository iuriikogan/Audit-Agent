...
func runBatch(ctx context.Context, client *genai.Client, apiKey, scope string) {
	...
	inputChan := make(chan domain.Resource, len(resources))
	...
	var finalResults []domain.AssessmentResult
...
func generateCSV(results []domain.AssessmentResult) {
	...
	writer.Write([]string{"ResourceID", "Name", "Type", "ComplianceStatus", "Tags"})
	for _, r := range results {
		...
		writer.Write([]string{r.ResourceID, r.ResourceName, r.ResourceType, status, r.Tags})
	}
	slog.Info("CSV Report generated", "file", "compliance_report.csv")
}

func generateTaggingInstructions(results []domain.AssessmentResult) {
...
func generateVisualReport(ctx context.Context, reporter agent.Agent, results []domain.AssessmentResult) {
